...
@test "simprod plan manages its memory correctly" {
    run $valgrind ./simprod plan
    assert_success
}

@test "simprod plan plan.json manages its memory correctly" {
    run $valgrind ./simprod plan examples/plan.json
    assert_success
}
